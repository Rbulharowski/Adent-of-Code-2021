...
# with open('editedmath.txt', 'w') as g:
#    g.writelines(changedtxt)
# #     print("\" Hello World \"")
snailfishwork = [[6,[[5,[7,7]],[[8,2],2]]],
...
#eval,test[0:len(test)-3] , "["+str(items)+"]"
workspace = []
sumitems = []
indextracker = []
...
base = "workspace"
manipulate = base
transfer = snailfishwork[0]
workspace = transfer.copy()
snailfishwork.pop(0)
print(workspace)

def listsearch(newindex):
  global workspace, indextracker, sumitems,base
  indextracker.append(object)
  while indextracker[len(indextracker)-1] < len(eval(manipulate)):
    manipulate = base
    for i in range(len(indextracker)):
      manipulate += "[" + str(indextracker[i]) + "]"
    if isinstance(eval(manipulate),list):
      isinstance(0)
    elif len(indextracker) > 3
    
for i in range(len(snailfishwork)):
  transfer = snailfishwork[i]
  print(transfer)
  workspace.append(transfer.copy())
  print(workspace)
  listsearch(0)
